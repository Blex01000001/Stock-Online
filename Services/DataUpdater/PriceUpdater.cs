...
namespace Stock_Online.Services.DataUpdater
{
    public class PriceUpdater : BaseDataUpdater
    {
        ...
        public override async Task UpdateAsync(string stockId, int year)
        {
            ...
            await ReportProgressAsync($"更新中 {stockId} 股價 {year}");
            ...
            await ExecuteWithRetryAsync(
                ...
                onRetryFail: (retry, ex) =>
                {
                    ...
                    return ReportProgressAsync($"⚠ 股價重試失敗 {stockId} 第{retry}次：{ex.Message}");
                },
                maxRetry: 3,
                delaySeconds: 5
            );

            await ReportProgressAsync($"✅ 股價更新完成 {stockId} {year}");
        }
